...
//            if (smsNum.equals(redisTemplate0.opsForValue().get(SMSTypeEnum.LOGIN.getSmsType() + phone))) {
    ...
    public ApiResponse smsRegister(String phone, Integer phoneType, String smsNum,
        ...
        String id_U = qt.getId_U(phone);
        if (id_U.equals("")) { //REGISTER
            //register starts here:
            // 设置info信息
            JSONObject info = qt.setJson("wrdN", wrdN, "pic", pic, "mbn", phone, "phoneType", phoneType);

            // 判断
            if (clientType.equals("app") && !("".equals(id_APP) || null == id_APP)) {
                info.put("id_APP", id_APP);
            } else {
                info.put("id_APP", "");
            }
            // 调用注册用户方法
            id_U = registerUserUtils.registerUser(info);
...
//            }
        ...
        if (!result.getJSONObject("infoData").getString("id_APP").equals(id_APP)) {
...
//                && s.length > 1 && s[1].equals(hz)
    ...
    private boolean isRegOff(String phone){
