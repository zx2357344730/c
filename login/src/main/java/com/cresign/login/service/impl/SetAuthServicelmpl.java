...
//        if (!qt.isNotNull(asset.getRole(), qt.setArray("role.objData."+ grpU + "."+listType)))
    ...
    public ApiResponse switchComp(String id_U, String id_C, String clientType) {
        ...
        // 通过id_U查询该用户
        User user = qt.getMDContent(id_U, Arrays.asList("rolex."+ id_C, "info"), User.class);
        //  here delete old Token,
        JSONObject userRolex = user.getRolex().getJSONObject(id_C);
        ...
        if (userRolex != null) {
            ...
            {
                updateData.put("rolex."+id_C+".wrdNC", comp.getInfo().getWrdN());
                updateData.put("rolex."+id_C+".picC", comp.getInfo().getPic());
...
//    @Transactional(rollbackFor = Exception.class, noRollbackFor = ResponseException.class)
    ...
    public ApiResponse getMySwitchComp(String id_U, String lang) {
        ...
        User one = qt.getMDContent(id_U, "rolex", User.class);
        ...
        // 最终返回数据
        JSONArray result = new JSONArray();
        JSONObject compList = one.getRolex();
        ...
        for (String cid :compList.keySet()) {
            if (compList.getJSONObject(cid).getString("picC") == null) {
                ...
                result.add(newDocs);
                qt.setMDContent(id_U, qt.setJson("rolex."+cid+".wrdNC", comp.getInfo().getWrdN(),
                        "rolex."+cid+".picC", comp.getInfo().getPic()), User.class);
    ...
    public void addRoleCompInfo(String id_C, String id_U,String grpU,String picC,JSONObject wrdN){
        JSONObject thisCompInfo = new JSONObject();
        thisCompInfo.put("grpU",grpU);
        thisCompInfo.put("id_C",id_C);
        thisCompInfo.put("dep","1000");
        thisCompInfo.put("picC",picC);
        thisCompInfo.put("wrdNC",wrdN);
        ...
        thisCompInfo.put("modAuth",modAuth);
        qt.setMDContent(id_U,qt.setJson("rolex."+id_C,thisCompInfo), User.class);
    }
