...
public class FlowServiceImpl implements FlowService {
    ...
    //New dgCheck, dgProcess, getDgResult,

    // Here important is CheckInfo, TaskLoop, PartInfo/SubInfo, infoPart?, timeHandle
...
////            result.put("grpGroup",grpGroup);
    ...
    /**
     * getDgSingle 自己的oItem+id_P 就可以只展开这个, 是protocol 的操作, 有用
     * @param id_OParent
     * @param index
     * @param id_U
     * @param myCompId
...
//                        salesOrderData.getOItem().getJSONArray("objItem").getJSONObject(index).getJSONObject("wrdN").getString("cn"));
                ...
                //TODO KEV use checkCard to init and then put in oItem, then use summOrder to fix all + lSBOrder
...
//            //System.out.println("bmdpt:"+objOItem.getBmdpt());
            ...
            orderODate.setCsSta(timeHandleSerialNoIsOneInside);
            // 设置订单时间操作信息
            orderODate.setWntDur(partInfo.getLong("wntDur")==null?120:partInfo.getLong("wntDur"));
            orderODate.setWntPrep(partInfo.getLong("wntPrep")==null?60:partInfo.getLong("wntPrep"));
            // action里面的
            //++ZJ
    ...
    public Object recursionInfoPart(JSONArray arrayObjItem, JSONObject stat) {
        ...
        for (int i = 0; i < arrayObjItem.size(); i++) {
            ...
            if (jsonProdPart.getJSONObject(jsonObjItem.getString("id_I")) != null) {
                ...
                jsonChildren.put("children", recursionInfoPart(arrayPartObjItem, stat));
            }
            arrayChildren.add(jsonChildren);
        }
        return arrayChildren;
...
//                id_P,Arrays.asList("info","part"));
    ...
    private void updatePartInfo(String id_C,JSONArray partItem,String id_P, JSONObject stat){
        for (int item = 0; item < partItem.size(); item++)
        {
                ...
                if (partDataES.size() == 0)
                {
                    //not exist
                    throw new ErrorResponseException(HttpStatus.OK, ErrEnum.PROD_NOT_FOUND.getCode(), "产品需要更新");
                }
...
//        }
        ...
        for (int i = 0; i < oDataList.size(); i++) {
            ...
            for (int j = 0; j < actionData.size(); j++) {
                if (actionData.getJSONObject(j).getInteger("bmdpt") == null)
                {
                    actionData.remove(j);
                }

                else if (actionData.getJSONObject(j).getInteger("bmdpt").equals(3)) {
