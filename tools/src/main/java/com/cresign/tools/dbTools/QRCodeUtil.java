...
public class QRCodeUtil {
    ...
    public static String getPdfQrCode(String content, String path, Integer size) {
        try {
            //图片类型
            ...
            //生成二维码存放文件
            path = new StringBuffer(path).append(System.currentTimeMillis()).append(random.nextInt(1000)).append(".jpg").toString();
     ...
     * @param logoPath logo的存放位置
     */
    public String zxingCodeCreateTang(String content, String path, Integer size, String logoPath, String id) {
        try {
            //图片类型
            String imageType = "jpg";
            //获取二维码流的形式，写入到目录文件中
            ...
            // 调用腾讯云工具上传文件
            // 并返回url给前端
            cos.uploadPE(file, path+"/", id, 0, null);
            if (file.exists()) {
                file.delete();
            }
